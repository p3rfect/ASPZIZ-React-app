...
const Exams = ({emailSearch, setFilled}) => {
    ...
    useEffect(() => {
        const getExams = async () => {
            try{
                const responseData = await getUserExams((emailSearch !== undefined ? emailSearch : email))
                await setExams([...responseData])
                if (responseData[0].type !== '') await setFilled(true)
            } catch (e) {
                console.log(e)
    ...
    const handleSubmit = async () => {
        try {
            if (validateExams()) {
                await postUserExams(exams, (emailSearch !== undefined ? emailSearch : email))
