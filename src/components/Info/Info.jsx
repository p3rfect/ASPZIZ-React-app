...
const validationSchema = yup.object({
    ...
    DocumentNumber: yup.string().length(6, 'Длина должна быть равна 6').matches(/^[0-9]+$/, 'Поле должно состоять только из цифер'),
...
const Info = ({emailSearch}) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                await updateUserInfo(values, (emailSearch !== undefined ? emailSearch : email))
    ...
    useEffect(() => {
        const fetchInfo = async () => {
            try {
                ...
                await formik.setValues(await getUserInfo((emailSearch !== undefined ? emailSearch : email)))
